...
def generate_hills(width, height, nhills):
    ...
    # generate hilltops
    xm, ym = np.mgrid[xmin+2:xmax:width/np.sqrt(nhills), ymin:ymax:height/np.sqrt(nhills)]
...
def save_heightfield(x, y, hfield, fname, path=None):
    ...
    plt.yticks([])
    plt.xticks([])
    plt.savefig(os.path.join(path, fname), bbox_inches='tight', pad_inches=0, frameon=False)
    plt.close()
