...
class ConjugateGradientOptimizer(Serializable):
    ...
    def update_opt(self, loss, target, leq_constraint, inputs, extra_inputs=None, constraint_name="constraint", *args,
        ...
        add_loss = kwargs.get('add_loss', None)
        if add_loss is None:
            ...
            self._opt_fun = ext.lazydict(
                ...
                f_add_loss=lambda: ext.compile_function(
                    inputs=inputs + extra_inputs,
                    outputs=add_loss,
                    log_name="f_add_loss",
                ),
    ...
    def add_loss(self, inputs, extra_inputs=None):
        inputs = tuple(inputs)
        if extra_inputs is None:
            extra_inputs = tuple()
        return sliced_fun(self._opt_fun["f_add_loss"], self._num_slices)(inputs, extra_inputs)
    ...
    def optimize(self, inputs, extra_inputs=None, subsample_grouped_inputs=None):
        ...
        for n_iter, ratio in enumerate(self._backtrack_ratio ** np.arange(self._max_backtracks)):
            ...
            print("Line search iter", n_iter, ":", constraint_val, loss, loss_before)
